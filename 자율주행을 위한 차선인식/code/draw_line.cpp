...
void draw_line::draw_carline(Mat& img_line, vector<Vec4i> lines)
{
	...
	*/
	...
	vector<Point> allPts;
	allPts.push_back(Point(right_x1, y1));
	allPts.push_back(Point(left_x1, y1));
	allPts.push_back(Point(left_x2, y2));
	allPts.push_back(Point(right_x2, y2));
	vector<vector<Point>> arr;
	arr.push_back(allPts);
	Mat overlay = Mat::zeros(img_line.size(), img_line.type());
	fillPoly(img_line, arr, Scalar(0, 255, 100));
	addWeighted(img_line, 1, overlay, 0.5, 0, img_line); //Overlay it

	int i = 0;

	ptVanish = lineLineIntersection(Point(right_x1, y1), Point(right_x2, y2), Point(left_x1, y1), Point(left_x2, y2));
	drawMarker(img_line, ptVanish, Scalar(0, 0, 255), 10000);
	Point ptCenter(img_line.cols / 2 - 1, img_line.rows / 2 - 1);
	arrowedLine(img_line, ptCenter, Point(ptVanish.x, ptCenter.y), Scalar(0, 255, 255), 2);

	int LR = ptVanish.x - img_line.cols / 2 - 1;
	std::string strIndex;
	if (LR<10 && LR > -10) {
		strIndex = "CENTER";
	}
	...
	putText(img_line, strIndex, Point(100, 100), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255));
}

Point draw_line::lineLineIntersection(Point A, Point B, Point C, Point D)
{
